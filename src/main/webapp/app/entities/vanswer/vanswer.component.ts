...
export class VanswerComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: IVanswer) {
        return item.id;
    }

    registerChangeInVanswers() {
        this.eventSubscriber = this.eventManager.subscribe('vanswerListModification', response => this.loadAll());
    }
    ...
    protected paginateVanswers(data: IVanswer[], headers: HttpHeaders) {
