...
-   \*_Progress_ - Fixes rounding error in precision settings _Thanks @aaroncox_
...
-   \*_Image_ - Added `hidden image` state
...
-**Docs** -**Transition** - `useFailSafe` was incorrectly shown as `false` by default
...
-   **Project** - Right-to-left (RTL) support added. New gulp tasks for RTL file generation and install setting. Docs however do not yet support RTL._Thanks @MohammadYounes for constant support with RTL!_.
...
-   **Build** - `Dist/` files now set file permissions in build. `644` by default. Can adjust in `semantic.json` or during gulp install. You will need to run `npm install` to add the new gulp-chmod dependency _Thanks @PeterDaveHello_
...
-   **Segment** - `ui tabular menu` now correctly aligns with attached segment when using fluid variation _Thanks @MohammadYounes_
...
-   **Menu** - Fixes dropdown formatting when used \*_inside_ a menu item
...
-   **Button** - Or buttons can now have text specified using `<div class="or" data-text"text">` with alternate text _Thanks MohammadYounes_
...
-   **Modal** - Fixes an issue where modal may not position correctly in some cases _Thanks GianlucaGuarini_
-   **Modal** - Fixes modal throwing an error when transition is not included _Thanks robertoles_
